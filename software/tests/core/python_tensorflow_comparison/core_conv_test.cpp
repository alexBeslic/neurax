...
int main(int argc, char* argv[]) {
    if (argc != 7) {
        std::cerr << "Upotreba: " << argv[0]
                  << " <batch> <H> <W> <in_channels> <out_channels> <kernel_size>\n";
        return 1;
    }
    ...
    size_t out_channels = std::stoul(argv[5]);
    size_t kernel_size  = std::stoul(argv[6]);
    ...
    // --- Gradnja mreže ---
    Conv2dBuilder conv2d = LayerBuilder::conv2d();
    conv2d.kernelSize(kernel_size)
          .stride(1)
          .padding(1)   // SAME padding
          .inputChanels(in_channels)
          .outputChanels(out_channels);
    conv2d.addWeights(weights_tensor, Tensor());

    NeuralNetworkBuilder builder;
    builder.addLayer(conv2d.build());
