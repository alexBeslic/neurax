...
entity neurax_register_block is
    ...
    port (
        ...
        neurax_start_o           : out std_logic;
        neurax_layer_enable_o    : out std_logic_vector(2 downto 0);
        neurax_pooling_type_o    : out std_logic_vector(1 downto 0);
        neurax_activation_type_o : out std_logic_vector(1 downto 0);
        neurax_16bit_mode_o      : out std_logic;
        neurax_finished_i        : in  std_logic
    );
...
begin
    ...
    begin
        if rst_i = '1' then
            ...
            ram(to_integer(c_REG_STATUS))(c_STATUS_FINISHED) <= neurax_finished_i;

            if avs_chipselect_i = '1' and avs_write_i = '1' then
                if unsigned(avs_address_i) < 2**g_ADDR_WIDTH and unsigned(avs_address_i) /= c_REG_READ_ONLY then
                    -- Valid address range
    ...
    --- Neurax Register Interface
    neurax_start_o <= ram(to_integer(c_REG_CMD))(c_CMD_START);
    neurax_layer_enable_o <= ram(to_integer(c_REG_CONFIG))(t_LAYER_ENABLE);
    neurax_pooling_type_o <= ram(to_integer(c_REG_CONFIG))(t_POOLING_TYPE);
    neurax_activation_type_o <= ram(to_integer(c_REG_CONFIG))(t_ACTIVATION_TYPE);
    neurax_16bit_mode_o <= ram(to_integer(c_REG_CONFIG))(c_16BIT_MODE);
end arch;